...
public class MainActivity extends AppCompatActivity implements OnMapReadyCallback , PopupMenu.OnMenuItemClickListener {
    ...
    private LatLng mNextDevicelocation;
    private LatLng mNextDestination;

    private Polyline mPolyline;
    private LatLng mDevicelocation;
    private List<LatLng> mDummyDevicelocation;
    private List<Integer> disanceFromOrigin;
  ...
  //              EditText edittext = (EditText) findViewById(R.id.editTextTextAddress);
   ...
   //             GeocodeConverter geocodeConverter = new GeocodeConverter();
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        {
            case 19:
                mDevicelocation=mNextDevicelocation;
    ...
    public List<LatLng> GetDummyLocations() {
    ...
    public LatLng GetADummy(LatLng latlngVal) {
       ...
       // double randomLat = ThreadLocalRandom.current().nextDouble(RoundInteger(latlngVal.latitude) , RoundInteger(latlngVal.latitude) + 1);
        //double randomLng = ThreadLocalRandom.current().nextDouble(RoundInteger(latlngVal.longitude), RoundInteger(latlngVal.longitude) + 1);
         double randomLat = ThreadLocalRandom.current().nextDouble(latlngVal.latitude-0.1 , latlngVal.latitude +0.1 );
        double randomLng = ThreadLocalRandom.current().nextDouble(latlngVal.longitude-0.1, latlngVal.longitude+0.1)  ;
    ...
    public LatLng GetMyDeviceLocation() {
        ...
        if (!isNetworkEnabled) {
            ...
            if (checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED && checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
             ...
             mLocationManager.requestLocationUpdates(LocationManager.NETWORK_PROVIDER, 100000, 0, mLocationListener);
            ...
            if (Devicelocation == null) {
                ...
                mLocationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 100000,  0, mLocationListener);
...
private String getNearestRoadsUrl(List<LatLng> lstLatLng)
{
    ...
    private class ParserTask extends AsyncTask<String, Integer, List<DirectionJSONWithDistance>> {
        ...
        @Override
        protected  List<DirectionJSONWithDistance> doInBackground(String... jsonData) {
   //     protected List<List<HashMap<String, String>>> doInBackground(String... jsonData) {
            JSONObject jObject;
          //  List<List<HashMap<String, String>>> routes = null;
            List<DirectionJSONWithDistance> routes = null;
            try {
        ...
        protected void onPostExecute(List<DirectionJSONWithDistance> result) {
            //test
...
//                List<HashMap<String, String>> path = result.get(i);
                DirectionJSONWithDistance pathWithDistance = result.get(i);
                List<HashMap<String, String>>  path=pathWithDistance.LocationPoints;
...
//                    Log.d("Insiede Zoom with device","" );
//                    ZoomToROute(lst);
//                }
//                else
                    if(mNextDestination!=null && mNextDevicelocation!=null)
                {
                    lst.add(mNextDevicelocation);
                    lst.add(mNextDestination);
                    if(mDummyDevicelocation.size()>0)
                        lst.addAll(mDummyDevicelocation);
                    Log.d("Insiede Zoom with dummy","" );
                    ZoomToROute(lst);
                }
            ...
            if(checkIfActualLocation(this))
            {
                getNextDeviceLocation(result);
            }
            mDestination=null;
        }
    }

public boolean checkIfActualLocation(ParserTask objParserTask) {
    if (objParserTask.startLng.latitude != mDevicelocation.latitude || objParserTask.startLng.longitude != mDevicelocation.longitude) {
        return false;
    }
    return true;
}
    private class GeocodeConverter extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... strAddress) {
            ...
            try {
                // Fetching the data from web service
                //data = downloadUrl(url[0]);
                // String url="https://maps.googleapis.com/maps/api/geocode/json?address=9200%20ivalenes%20hope%20dr&key=[KeyEnter]";
        ...
        protected void onPostExecute(String result) {
            ...
            try {
                ...
                for (int i = 0; i < jResults.length(); i++) {
                    ...
                    mNextDestination=new LatLng(strLat, strLong);
...
//              // mDummyDevicelocation.addAll(lstLatlng);
    ...
    public LatLng getNextDeviceLocation(List<DirectionJSONWithDistance> lstDirectionJSONWithDistance) {
        int MilesToCHeck = 5000;
        int TotalDistanceFromDevice = 0;
        for (int i = 0; i < lstDirectionJSONWithDistance.size(); i++) {
            DirectionJSONWithDistance objDirectionJSONWithDistance = lstDirectionJSONWithDistance.get(i);
            TotalDistanceFromDevice +=   objDirectionJSONWithDistance.distanceinMeters;
            if (TotalDistanceFromDevice > MilesToCHeck) {
                List<HashMap<String, String>>  path= objDirectionJSONWithDistance.LocationPoints;
                HashMap<String, String> point = path.get(path.size()-1);
                double lat = Double.parseDouble(point.get("lat"));
                double lng = Double.parseDouble(point.get("lng"));
                mNextDevicelocation= new LatLng(lat, lng);
                break;
            }
        }
        return mNextDevicelocation;
    }
}